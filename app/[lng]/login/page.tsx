...
// Nous chargeons les données en dehors du rendu principal si nécessaire,
// ou nous forçons une structure que Next.js compile parfaitement.
export default async function LoginPage(props: PageProps) {
  const { lng } = await props.params;

  // Importation dynamique simplifiée pour éviter le conflit d'export Turbopack
  const messagesModule = await import(`../../../messages/${lng}.json`);
  const t = messagesModule.default.Auth.login;

  return (
    <div className="min-h-screen bg-[#f4f7f6] flex items-center justify-center p-4 sm:p-6 lg:p-8 font-sans">
      <div className="bg-white rounded-[24px] shadow-xl overflow-hidden w-full max-w-4xl flex flex-col md:flex-row min-h-[580px] border border-gray-100">
        ...
        <div className="md:w-1/2 p-4 sm:p-8 lg:p-12 flex items-center justify-center bg-white">
          <SignIn
            routing="hash"
            signUpUrl={`/${lng}/register`}
            fallbackRedirectUrl={`/${lng}/onboarding`} // Redirige vers onboarding après connexion aussi au cas où !
            appearance={{
              elements: {
